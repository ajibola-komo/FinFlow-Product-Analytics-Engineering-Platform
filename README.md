...
| `dim_event_type` | dimension | One row per distinct event type | 12 rows |
| `dim_product` | dimension | One row per financial product offering | 2 rows |
| `dim_plan` | dimension | One row per product plan variant | 4 rows |
| `dim_user` | dimension | One row per registered user | ~2,000,000 rows |
| `dim_wallet` | dimension | One row per user wallet account (one wallet per user) | ~2,000,000 rows |
| `fact_event` | fact | One row per event occurrence generated by a user | ~4,500,000 rows |
