...
| `dim_product` | dimension | One record per financial product offering | 2 rows |
| `dim_plan` | dimension | One record per product plan variant | 4 rows |
| `dim_user` | dimension | One record per registered user | ~2,000,000 rows |
| `dim_wallet` | dimension | One record per user wallet account (one wallet per user) | ~2,000,000 rows |
| `dim_transaction_type` | dimension | One record per money movement or transaction event type | ~10 rows |
| `fact_event` | fact | One record per event occurrence generated by a user | ~4,500,000 rows |
...
| `dim_wallet_state` | dimension | One record per wallet's latest state | ~2,000,000 rows |
| `dim_investment_position_state` | dimension | One record per active investment's latest state  | ~1,100,000 rows |
| `dim_user_state` | dimension| One record per active user's latest state  | ~2,000,000 rows |
